...
public class UserView extends JPanel {
    ...
    private SearchPanel createSearchPanel() {
        return new SearchPanel("Tìm người dùng:", 22, () -> runWithController(UserController::onSearch));
    }
    ...
    private JPanel createActionPanel() {
        ...
        addButton.setToolTipText("Thêm người dùng");
        addButton.setIcon(tintIcon(loadIcon("/icon/plus.png", ICON_SIZE), Color.BLACK));

        importButton.addActionListener(event -> runWithController(UserController::onImport));
        refreshButton.addActionListener(event -> runWithController(UserController::onRefresh));
        addButton.addActionListener(event -> runWithController(UserController::onAdd));
    ...
    private void setupActionColumns() {
        ...
        table.getColumnModel().getColumn(ACTION_COLUMN_INDEX).setMaxWidth(200);
        table.getColumnModel().getColumn(ACTION_COLUMN_INDEX).setMinWidth(200);
        table.getColumnModel().getColumn(ACTION_COLUMN_INDEX).setPreferredWidth(200);
    ...
    private void runWithController(java.util.function.Consumer<UserController> action) {
        if (controller != null) {
    ...
    private final class ActionCellRenderer extends JPanel implements TableCellRenderer {
        ...
        private ActionCellRenderer(ImageIcon viewIcon, ImageIcon editIcon, ImageIcon deleteIcon) {
            ...
            toggleButton = createIconButton(null, "Bật/Tắt", "On/Off");
    ...
    private final class ActionCellEditor extends AbstractCellEditor implements TableCellEditor {
        ...
        private ActionCellEditor(ImageIcon viewIcon, ImageIcon editIcon, ImageIcon deleteIcon) {
            ...
            JButton toggleButton = createIconButton(null, "Bật/Tắt", "On/Off");
            ...
            toggleButton.addActionListener(event -> handleActionRow(ActionType.TOGGLE));
            ...
            if (row >= 0) {
                runWithController(ctrl -> {
                    switch (type) {
                        ...
                        case TOGGLE -> ctrl.onToggleRow(row);
                    }
                });
            }
            fireEditingStopped();
